...
const ManagerApp = (() => {
  ...
  async function initialize() {
    ...
    if (!['manager', 'staff'].includes(auth.user.role)) {
      alert('Bạn không có quyền truy cập trang này. Chỉ quản lý và nhân viên mới có thể truy cập.');
  ...
  function populateFacilityDropdowns() {
    const dropdown = document.getElementById("mgr-facility-select-global");
  ...
  function switchTab(tabName) {
    ...
    if (tabName === "courts") {
      ...
      if (window.ManagerSettings) {
        window.ManagerSettings.render(cachedCourts);
      }
    }
  }
  ...
  async function refreshCourtsTab(forceRefresh = false) {
    ...
    try {
      ...
      if (currentTab === "settings" && window.ManagerSettings) {
        window.ManagerSettings.render(cachedCourts);
      }
  ...
  function setupEventListeners() {
    ...
    document.getElementById("mgr-facility-select-global")?.addEventListener("change", (e) => {
    ...
    document.getElementById("mgr-date-select-global")?.addEventListener("change", (e) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ManagerApp.initialize().catch((err) => {
    console.error("[ManagerApp] Initialization error:", err);
  });
});
